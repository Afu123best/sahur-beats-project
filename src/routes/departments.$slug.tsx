    ...
    <PublicLayout>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="services" className="mt-6 grid gap-3 sm:grid-cols-2">
            {dep.services.map((s: string) => (
              <div key={s} className="flex items-center gap-3 rounded-xl border bg-card p-4"><CheckCircle2 className="h-4 w-4 text-primary" />{s}</div>
            ))}
          </TabsContent>
          ...
          <TabsContent value="facilities" className="mt-6 grid gap-3 sm:grid-cols-2">
            {dep.facilities.map((f: string) => (
              <div key={f} className="rounded-xl border bg-card p-4">{f}</div>
            ))}
          </TabsContent>
          <TabsContent value="equipment" className="mt-6 grid gap-3 sm:grid-cols-2">
            {dep.equipment.map((f: string) => (
              <div key={f} className="rounded-xl border bg-card p-4">{f}</div>
            ))}
          </TabsContent>
